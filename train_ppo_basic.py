...
from ppo.ppo_trainer_basic import PPOTrainer
...
class GPT2RewardModel(GPT2LMHeadModel):
    def __init__(self, config, opt, tokenizer):
        ...
        self.opt = opt
        self.tokenizer = tokenizer
        self.reward_head = torch.nn.Linear(config.n_embd, 1, bias=False) # 보상 스칼라 하나 출력
    ...
    def forward(self, decoder_input, only_last=True):
        ...
        if only_last:
            logits = self.reward_head(hidden_states[:, -1, :]).squeeze(-1) # [B]
        else:
            logits = self.reward_head(hidden_states).squeeze(-1) # [B, T]
...
# quantile 분포 방식
class GPT2QuantileRewardModel(GPT2PreTrainedModel): # GPTPreTrainedModel 상속
    def __init__(self, config, opt, tokenizer):
        super().__init__(config)
        self.opt = opt
        self.tokenizer = tokenizer
        self.num_quantiles = 10 # quantile 개수

        self.transformer = GPT2Model(config) # transformer만 따로 직접 정의 (GPT2LMHeadModel 아님)
        self.quantile_head = nn.Linear(config.hidden_size, self.num_quantiles) # quantile 분포 예측
        self.post_init() # weights 초기화 등 처리 (PreTrainedModel 상속 시 필요)

    def forward(self, decoder_input, only_last=True):
        attention_mask = decoder_input.ne(self.tokenizer.pad_token_id)

        output = self.transformer( # GPT2Model에서 직접 last_hidden_state 출력
            input_ids=decoder_input,
            attention_mask=attention_mask,
            return_dict=True,
            use_cache=False
        ...
        if only_last:
            last_token_index = attention_mask.sum(dim=1) - 1  # [B], 실제 마지막 토큰 위치 계산
            last_hidden = hidden_states[torch.arange(hidden_states.size(0)), last_token_index]  # [B, H]
            quantiles = self.quantile_head(last_hidden)  # [B, N], quantile 값 출력
        else:
            quantiles = self.quantile_head(hidden_states)  # [B, T, N], 전체 시퀀스 분포 예측
