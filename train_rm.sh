...
accelerate launch --config_file accelerate_config_gpu1.yaml --num_processes 1 train_rm.py \
--hf_model_name_or_path outputs/models/sft/gpt_tuned_sft/checkpoint-1500/ \
--model_save_path outputs/rewardModel \
--batch_size 4 \
...
--gradient_checkpoint \
--delimiter '</s>' \
--logdir outputs/tensorboard_log/reward/reward_model_en \
&> outputs/log/reward_model_en.log
