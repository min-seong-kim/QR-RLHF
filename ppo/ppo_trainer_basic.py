...
class PPOTrainer():
    ...
    def make_experiences(self):
        ...
        while len(self.replay_buffer) < self.num_rollouts:
            ...
            # # reward quantile 적용
            # quantiles, *_ = self.reward_model_forward(sampled_vec)  # [B, T, N]
            # rewards = quantiles[:, -1, :].mean(dim=-1)        # 마지막 토큰 기준 [B, N] → mean → [B]


            # rewards = rewards.mean(dim=-1)  # [B], 리워드 quantile 평균으로
            
            rewards = rewards.cpu()

            self.train_metrics.record_metric_many('rewards', rewards.tolist())
            # # quantile 분포 통계 기록
            # self.train_metrics.record_metric('reward_quantile_std', quantiles[:, -1, :].std(dim=-1).mean().item())
    ...
    def evaluate(self, datatype='valid', **kwargs) -> Tuple[float, List]:
        ...
        for step, batch in enumerate(valid_dataloader):
            ...
            # 평균을 취하고 list로 들어가도록
            # rewards = self.reward_model_forward(output_vec)[0].mean(dim=-1).tolist()  # [B, N] → [B],
