...
class RewardTrainer():
    ...
    def _criterion(self, model_output: torch.Tensor, batch: Dict[str, Any], return_output=False, training=True):
        # model_output은 reward model의 출력값 (tuple 형태로 반환됨)
        # 첫 번째 원소는 reward 값 (logits), 나머지는 lm_logits (optional)
        logits, *outputs = model_output
        bs = logits.size(0) // 2
        
        # 쌍의 앞 절반은 chosen(선호 응답), 뒷 절반은 rejected(비선호 응답)
        preferred_rewards = logits[:bs] # shape: [B]
        rejected_rewards = logits[bs:]  # shape: [B]
        
        # 보상 차이를 sigmoid 함수에 통과시켜 확률로 변환
        # 값이 클수록 chosen이 rejected보다 reward가 높다는 의미
        probs = torch.sigmoid(preferred_rewards - rejected_rewards)
        
        # pairwise reward loss: cross entropy 기반 로그 손실
        # sigmoid(probs)가 1에 가까울수록 손실이 작음 (올바르게 판단한 것)
        ...
        if self.calculate_lm_loss:
            ...
            # chosen 응답에 대해만 lm loss 계산
            scores = lm_logits[:bs, :-1, :] # lm loss for chosen only, 예측한 다음 토큰 분포 (shifted)
            preds = scores.argmax(dim=-1) # 예측된 토큰 인덱스
            
            # 라벨 생성: 실제 다음 토큰
            label_vec = batch['text_vec'][:bs, 1:].clone() # 정답은 한 칸 오른쪽
            loss_mask = batch['loss_mask'][:, 1:] # 패딩 제외용 마스크
            label_vec[~loss_mask] = self.tokenizer.pad_token_id # 무시할 토큰은 pad로 채움
            batch['label_vec'] = label_vec # 디버깅이나 분석용 저장
            
            # LM loss 계산 (CrossEntropy 기준)
            lm_loss = self._lm_loss(scores, preds, label_vec, training)
            loss = loss + self.lm_loss_factor * lm_loss # 최종 loss에 가중합 형태로 포함
            
        with torch.no_grad():
            # 확률이 0.5보다 크면 맞췄다고 간주 (accuracy 측정)
            correct = (probs > .5).float().mean()
            metrics = self._get_metric_obj(training)
            metrics.record_metric('rm_loss', loss.item()) # reward model loss
            metrics.record_metric('rm_acc', correct.item()) # reward model 정확도
    ...
    def evaluate(self, datatype='valid', **kwargs) -> Tuple[float, List]:
        ...
        scalar_reward_preferred, scalar_reward_rejected = [], []
        ...
        for step, batch in enumerate(valid_dataloader):
            ...
            if isinstance(model_output, tuple) and model_output[0].dim() == 1:
                reward = model_output[0]
                bs = reward.size(0) // 2
                preferred_q = reward[:bs].detach().to(torch.float32).cpu().numpy()
                rejected_q = reward[bs:].detach().to(torch.float32).cpu().numpy()

                scalar_reward_preferred.append(preferred_q)
                scalar_reward_rejected.append(rejected_q)
        ...
        # CDF용 데이터 저장
        preferred_full = np.concatenate(scalar_reward_preferred, axis=0)   
        rejected_full = np.concatenate(scalar_reward_rejected, axis=0) 
        
        step_str = str(self.train_state.total_steps)
        save_dir = 'outputs/csv'

        pd.DataFrame(preferred_full).to_csv(f"{save_dir}/preferred_rewards_scalar_step{step_str}.csv", index=False)
        pd.DataFrame(rejected_full).to_csv(f"{save_dir}/rejected_rewards_scalar_step{step_str}.csv", index=False)
